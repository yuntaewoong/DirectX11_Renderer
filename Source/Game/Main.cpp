...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    // Environment Map
    std::shared_ptr<library::VertexShader> environmentMapVertexShader = std::make_shared<library::VertexShader>(L"Shaders/EnvironmentShaders.fxh", "VSEnvironmentMap", "vs_5_0");
    ...
    // Environment Map
    std::shared_ptr<library::PixelShader> environmentMapPixelShader = std::make_shared<library::PixelShader>(L"Shaders/EnvironmentShaders.fxh", "PSEnvironmentMap", "ps_5_0");
    ...
    floorCube->Scale(150.0f, 0.1f, 150.0f);
    ...
    pointLight->Translate(XMVectorSet(0.0f, 30.0f, 0.0f, 1.0f));
    ...
    EnvironMentCube->Translate(XMVectorSet(0.0f, 5.0f, 0.0f, 1.0f));
    if (FAILED(mainScene->AddRenderable(L"EnvironMentCube", EnvironMentCube)))
    {
        return 0;
    }
    if (FAILED(mainScene->SetVertexShaderOfRenderable(L"EnvironMentCube", L"EnvironmentMapShader")))
    {
        return 0;
    }
    if (FAILED(mainScene->SetPixelShaderOfRenderable(L"EnvironMentCube", L"EnvironmentMapShader")))
    {
        return 0;
    }
