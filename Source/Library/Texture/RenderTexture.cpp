...
namespace library
{
	...
	RenderTexture::RenderTexture(_In_ UINT uWidth, _In_ UINT uHeight) :
		m_uWidth(uWidth),
		m_uHeight(uHeight),
		m_texture2D(ComPtr<ID3D11Texture2D>()),
		m_renderTargetView(ComPtr<ID3D11RenderTargetView>()),
		m_shaderResourceView(ComPtr<ID3D11ShaderResourceView>()),
		m_samplerClamp(ComPtr<ID3D11SamplerState>())
	{}
